...
def get_change_over_months(request, *args, **kwargs):
    format = "%Y-%m-%dT%H:%M:%S"
    try:
        from_time = datetime.strptime(request.GET.get('from'), format)
    except:
        from_time = None
    try:
        to_time = datetime.strptime(request.GET.get('to'), format)
    except:
        to_time = None
    ...
    if (from_time == None):
        from_time = TickModel.objects.aggregate(Min('date'))['date__min']
        from_time = datetime.strptime(datetime.strftime(from_time, format), format)
    if (to_time == None):
        to_time = TickModel.objects.aggregate(Max('date'))['date__max']
        to_time = datetime.strptime(datetime.strftime(to_time, format), format)
