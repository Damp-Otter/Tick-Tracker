...
I will explain what measures I took to hankle errors by the user and the server.

### Api failures:

-   I created a custom 404 error page that you are taken to if you enter 
    the url incorrectly or provide an invalid error. The page states what went wrong and lists hyperlinks to all valid url paths. 
...
-   Every field except latin-name in the TickModel has null and blank set
    to true, because they are used to filter the data and must not be missing.
-   Assuming every species has only one latin_name, I changed the save 
    function so all latin_names are the same as the first record of latin_name within the same species. This is obviously not a perfect system, but the species will map directly onto latin_name as a one-to-one mapping.
